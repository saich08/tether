...
export function FileEditorDialog({
  ...
  file,
  onClose,
}: FileEditorDialogProps): JSX.Element {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setError(null);
    window.electron.sftp
      .readFile({ connectionId, path: file.path })
      .then((result) => {
        if (cancelled) return;
  ...
  const handleSave = useCallback(async () => {
    ...
    setSaving(true);
    setSaveError(null);
    const result = await window.electron.sftp.writeFile({
      connectionId,
      path: file.path,
      content,
    });
          ...
          <div className="flex items-center gap-2.5 min-w-0">
            ...
            <div className="min-w-0">
              <div className="flex items-center gap-1.5">
                <span className="text-sm font-semibold text-surface-100 font-mono truncate">
                  {file.name}
                </span>
                {isDirty && (
                  <span
