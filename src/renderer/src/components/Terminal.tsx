...
export const Terminal = forwardRef<TerminalHandle, TerminalProps>(
  function Terminal({ connectionId }, ref): JSX.Element {
    ...
    const openShell = useCallback(
      async (term: TerminalInstance, fitAddon: FitAddon) => {
        ...
        const result = await window.electron.terminal.openShell(
          connectionId,
          dims,
        );
        if (!result.ok) {
    ...
    useEffect(() => {
      ...
      const removeDataListener = window.electron.terminal.onData(
        ({ connectionId: cid, data }) => {
          if (cid === connectionId) {
            term.write(data);
          }
        },
