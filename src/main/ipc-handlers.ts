...
export function registerIpcHandlers(manager: SSHManager, getWindow: () => BrowserWindow | null): void {
  // ─── SSH Connect ──────────────────────────────────────────────────────────
  ...
  ipcMain.handle(
    ...
    async (
      ...
      try {
        await manager.openShell(payload.connectionId, payload.dimensions, (data) => {
          const win = getWindow()
          if (win) {
  ...
  ipcMain.on(
    IPC.TERMINAL_RESIZE,
    (_event, payload: { connectionId: string; dimensions: TerminalDimensions }) => {
  ...
  ipcMain.handle(
    ...
    (_event, { connectionId, path }: { connectionId: string; path: string }): IPCResult => {
      const session = manager.getSession(connectionId)
      if (!session) return { ok: false, error: 'No active session' }
      const { username, host, port } = session.connection.credentials
      const authority = port === 22 ? `${username}@${host}` : `${username}@${host}:${port}`
      const child = spawn('code', ['--remote', `ssh-remote+${authority}`, path], {
        detached: true,
        stdio: 'ignore'
      })
      child.unref()
